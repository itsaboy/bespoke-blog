...
export default function ChangeHome() {
  ...
  const uploadGalleryPagePostToServer = async () => {
    ...
    const attemptUpload = async () => {
      try {
        const checkResponse = await fetch("/api/galleryPagePost/check", {
          method: "GET",
          credentials: "include",
        });
        if (checkResponse.ok) {
          ...
          if (exists && id) {
            await fetch(`/api/galleryPagePost/delete/${id}`, {
              method: "DELETE",
              credentials: "include",
            });
          }
        ...
        const response = await fetch("/api/galleryPagePost/create", {
          method: "POST",
          body: formData,
          credentials: "include",
        });
        ...
        if (error.message === "Authentication required") {
          try {
            ...
            if (refreshSuccess) {
              const retryResponse = await fetch("/api/galleryPagePost/create", {
                method: "POST",
                body: formData,
                credentials: "include",
              });
