...
export default function ChangeHome() {
  ...
  const uploadBlogPagePostToServer = async () => {
    ...
    const attemptUpload = async () => {
      try {
        const checkResponse = await fetch("/api/blogPagePost/check", {
          method: "GET",
          credentials: "include",
        });
        if (checkResponse.ok) {
          ...
          if (exists && id) {
            await fetch(`/api/blogPagePost/delete/${id}`, {
              method: "DELETE",
              credentials: "include",
            });
          }
        ...
        const response = await fetch("/api/blogPagePost/create", {
          method: "POST",
          body: formData,
          credentials: "include",
        });
        ...
        if (error.message === "Authentication required") {
          try {
            ...
            if (refreshSuccess) {
              const retryResponse = await fetch("/api/blogPagePost/create", {
                method: "POST",
                body: formData,
                credentials: "include",
              });
